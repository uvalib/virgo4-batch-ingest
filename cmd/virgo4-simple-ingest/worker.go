   ...
   for {
      ...
      if arrived == true {
         ...
         if blocksize == awssqs.MAX_SQS_BLOCK_COUNT {
            err := processesInboundBlock( id, aws, queued, outQueue )
            if err != nil {
               log.Fatal( err )
            }
...
func processesInboundBlock( id int, aws awssqs.AWS_SQS, messages []awssqs.Message, outQueue awssqs.QueueHandle ) error {
